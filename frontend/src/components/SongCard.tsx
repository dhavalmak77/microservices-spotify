	...
	return (
		<div className="min-w-[180px] p-2 px-3 rounded cursor-pointer hover:bg-[#ffffff26]">
			<div className="relative group">
				<img src={image ?? "/download.jpeg"} alt={name} className="mr-1 w-[160px] rounded" />
				<div className="flex gap-2">
					...
					{isAuth && (
						<button className="absolute bottom-2 right-2 bg-green-500 text-black p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300" onClick={() => addToPlaylist(id)}>
							<FaBookmark />
						</button>
					)}
				</div>
