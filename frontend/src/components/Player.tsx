...
const Player = () => {
	...
	useEffect(() => {
		...
		audio.addEventListener("loadmetadata", () => handleLoadedMetaData(audio));
			...
			{song && (
				<div className="h-[10%] bg-black flex justify-between items-center text-white px-4">
					<div className="lg:flex items-center gap-4">
						<img src={song.thumbnail ? song.thumbnail : '/download.jpeg'} alt="" className="w-12" />
					...
					<div className="flex flex-col items-center gap-1 m-auto">
						...
						<div className="w-full items-center flex font-thin text-green-400">
							<input type="range" min={0} max={100} className="progess-bar w-[120px] md:w-[300px]" value={(progress / duration) * 100 || 0} onChange={durationChange} />
						</div>
					...
					<div className="flex items-center">
						<input type="range" min={0} max={100} step={0.01} value={volume * 100} onChange={volumeChange} className="w-16 md:w-32" />
					</div>
				</div>
			)}
		</div>
